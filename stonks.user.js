...
// @downloadURL  https://raw.githubusercontent.com/sid-the-sloth1/torn-qol-scripts/main/stonks.user.js
// @grant        unsafeWindow
                ...
                if (data.result && data.result.channel && data.result.channel == "stockMarket") {
                    ...
                    if (msg.action == "refreshStocksPrice") {
                        ...
                        for (const stock of stocks) {
                            let id = stock.id;
                            let price = stock.chartsData[0][1].value;
                            metadata[id].price = price;
                            moneyInvested += Math.ceil(price*metadata[id].ownedShares);
                        }
                        stockLossObj = {};
                        stonksTotalVal = 0;
                        ...
                        if (pageurl.includes("link=hardy")) {
                            ...
                            addProfitLossInfo();
                            changeTitle();
                            document.querySelector(".hardy_stonks_text_info").innerHTML = `${stonksTotalVal >= 0?'<span class="stonksUpli">Current Profit: ':'<span class ="stonksDownli">Current Loss: '}${formatNumber(stonksTotalVal)}</span><br>Current Holdings: ${formatNumber(moneyInvested)}<br>Invested Into ${investedIn} Stocks`;
                        }
        ...
        if (url.includes("page.php?sid=StockMarket&step=getInitialData")) {
            respo.json().then((info) => {
                ...
                for (const stock of stocks) {
                    ...
                    metadata[id].ownedShares = userData.sharesAmount;
                    let price = stock.sharesPrice.chartData[1].value;
                    if (userData.sharesAmount > 0) {
                        investedIn += 1;
                        moneyInvested += Math.ceil(price*userData.sharesAmount);
                        portfolioData[id] = [];
            ...
            respo.json().then((info) => {
                if (info.success) {
                    ...
                    let oldShares = metadata[id].ownedShares;
                    if (oldShares === 0) {
                        investedIn += 1;
                    }
                    metadata[id].ownedShares += obj.amount;
            ...
            respo.json().then((info) => {
                if (info.success) {
                    ...
                    sendData(obj);
                    let oldShares = metadata[id].ownedShares;
                    let remainingShares = oldShares - obj.amount;
                    if (remainingShares === 0) {
                        investedIn = investedIn - 1;
                    }
                    metadata[id].ownedShares = remainingShares;
...
#clearFilter { border-style: solid; border-color: rgb(169, 169, 169); padding: 8px 8px; font-size: 16px; border-radius: 4px; margin: 6px 8px; background-color: #c52323; color: #f1ecec; }
    `);
})();
